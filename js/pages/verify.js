...
Pages.Verify = (() => {
  ...
  function _renderResults(onlyMismatch = false) {
    ...
    const rows = list.map((r,i) => {
      const stStyle  = r.result==='ok' ? 'border:1px solid var(--bd);color:var(--tx2);background:transparent' : r.result==='mismatch' ? 'border:1px solid #FECACA;color:#dc2626;background:#FEF2F2' : 'border:1px solid var(--bd);color:var(--tx2);background:transparent';
      ...
      const diffCell = r.diff===null ? '—' : r.diff===0 ? '—' : `<span style="border:1px solid #FECACA;color:#dc2626;background:#FEF2F2;font-family:var(--font-mono);font-size:12px;padding:1px 6px;border-radius:3px">${r.diff>0?'+':''}${formatNumber(Math.round(r.diff))}</span>`;
      return `
        <tr style="border-top:1px solid var(--tbl-row-bd);${rowBg}">
          <td style="padding:9px 13px;font-size:12px;color:var(--tbl-tx-body);text-align:center">${i+1}</td>
          <td style="padding:9px 13px;font-size:12px;color:var(--tbl-tx-body)">${r.date}</td>
          <td style="padding:9px 13px;font-family:var(--font-mono);font-size:12px">${r.lotNo}</td>
          <td style="padding:9px 13px">${r.country ? `<span style="display:inline-flex;align-items:center;font-size:12px;font-weight:500;padding:1px 6px;border-radius:3px;${CO_STYLE[r.country]||''}">${r.country}</span>` : '—'}</td>
          <td style="padding:9px 13px;text-align:right;font-family:var(--font-mono);font-size:12px">${r.proc>0?formatNumber(Math.round(r.proc)):'—'}</td>
          <td style="padding:9px 13px;text-align:right;font-family:var(--font-mono);font-size:12px;color:var(--tx2)">${r.dbProc!==null?formatNumber(Math.round(r.dbProc)):'—'}</td>
          <td style="padding:9px 13px;text-align:right">${diffCell}</td>
          <td style="padding:9px 13px"><span style="display:inline-flex;align-items:center;font-size:12px;font-weight:500;padding:2px 7px;border-radius:3px;${stStyle}">${stLabel}</span></td>
        </tr>`;
    }).join('');
    ...
    el.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1px;background:var(--bd);border-radius:var(--r);overflow:hidden;margin-bottom:12px">
        <div style="background:var(--tbl-sum-bg);padding:10px 14px;text-align:center">
          <div style="font-size:12px;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">총 비교</div>
        ...
        <div style="background:#E1F5EE;padding:10px 14px;text-align:center">
          <div style="font-size:12px;color:var(--tx);text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">일치</div>
          <div style="font-size:20px;font-weight:500;color:var(--tx)">${okCnt}건</div>
        </div>
        <div style="background:#FCEBEB;padding:10px 14px;text-align:center">
          <div style="font-size:12px;color:#791F1F;text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">불일치</div>
        ...
        <div style="background:#FAEEDA;padding:10px 14px;text-align:center">
          <div style="font-size:12px;color:#633806;text-transform:uppercase;letter-spacing:.05em;margin-bottom:3px">DB 없음</div>
          <div style="font-size:20px;font-weight:500;color:#633806">${missingCnt}건</div>
        </div>
      </div>

      <div style="background:var(--tbl-bg);border:1px solid var(--tbl-wrap-bd);border-radius:10px;overflow:hidden;margin-bottom:10px">
        <table style="width:100%;border-collapse:collapse;font-size:12px">
          <thead><tr style="background:var(--tbl-sum-bg)">
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd);width:32px;text-align:center">#</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd)">날짜</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd)">LOT/번호</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd)">지역</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd);text-align:right">리포트 ${typeLabel}</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd);text-align:right">DB ${typeLabel}</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd);text-align:right">차이</th>
            <th style="padding:9px 13px;font-size:12px;font-weight:500;color:var(--tbl-tx-body);text-transform:uppercase;letter-spacing:.05em;border-top:1px solid var(--tbl-row-bd)">결과</th>
          </tr></thead>
          <tbody>${rows||'<tr><td colspan="8" style="padding:20px;text-align:center;color:var(--tbl-tx-body)">결과 없음</td></tr>'}</tbody>
        </table>
      </div>

      <div style="display:flex;gap:8px;justify-content:flex-end">
        <button onclick="Pages.Verify.filterMismatch()" style="padding:6px 14px;border:1px solid var(--bd);border-radius:var(--rs);font-size:12px;background:none;color:var(--tx2);cursor:pointer">불일치만 보기</button>
        <button onclick="Pages.Verify.exportResults()" style="padding:6px 14px;border:1px solid var(--bd);border-radius:var(--rs);font-size:12px;background:none;color:var(--tx2);cursor:pointer">↓ 엑셀 내보내기</button>
        ${bulkBtn}
      </div>`;
  }
    ...
    el.innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">
        <div style="background:var(--card);border:1px solid var(--bd);border-radius:var(--r);padding:14px">
          <div style="font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;color:var(--tbl-tx-body);margin-bottom:8px;display:flex;align-items:center;gap:6px">
            <span style="width:18px;height:18px;border-radius:50%;background:var(--tx);color:var(--card);font-size:12px;font-weight:500;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0">1</span>
            검증 유형 선택
          </div>
          <div style="display:flex;gap:6px;margin-bottom:14px">
            <button id="vfy-type-daily" onclick="Pages.Verify.setType('daily')"
              style="padding:4px 12px;border-radius:20px;font-size:12px;font-weight:500;cursor:pointer;border:1px solid var(--navy);background:var(--navy);color:#fff;transition:.15s">일별 처리</button>
            <button id="vfy-type-lot" onclick="Pages.Verify.setType('lot')"
              style="padding:4px 12px;border-radius:20px;font-size:12px;font-weight:500;cursor:pointer;border:1px solid var(--bd2);background:none;color:var(--tx2);transition:.15s">LOT 목록</button>
            <button id="vfy-type-invoice" onclick="Pages.Verify.setType('invoice')"
              style="padding:4px 12px;border-radius:20px;font-size:12px;font-weight:500;cursor:pointer;border:1px solid var(--bd2);background:none;color:var(--tx2);transition:.15s">인보이스</button>
          </div>
          <div style="font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;color:var(--tbl-tx-body);margin-bottom:6px;display:flex;align-items:center;gap:6px">
            <span style="width:18px;height:18px;border-radius:50%;background:var(--tx);color:var(--card);font-size:12px;font-weight:500;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0">2</span>
            리포트 붙여넣기
          </div>
          <textarea id="vfy-paste" style="width:100%;height:120px;padding:10px 13px;border:0.5px solid var(--bd2);border-radius:var(--rs);font-size:12px;font-family:var(--font-mono);background:var(--tbl-sum-bg);color:var(--tx);resize:vertical;line-height:1.5"
...
2026-03-17	SGT100001597	320	280	25	15"></textarea>
        ...
        <div style="background:var(--card);border:1px solid var(--bd);border-radius:var(--r);padding:14px">
          <div style="font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;color:var(--tbl-tx-body);margin-bottom:8px;display:flex;align-items:center;gap:6px">
            <span style="width:18px;height:18px;border-radius:50%;background:var(--tx);color:var(--card);font-size:12px;font-weight:500;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0">3</span>
            이미지 업로드 (홍콩 캡처)
          </div>
          <div style="width:100%;height:90px;border:1px dashed var(--bd2);border-radius:var(--rs);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:5px;background:var(--tbl-sum-bg);color:var(--tbl-tx-body);font-size:12px;cursor:pointer;margin-bottom:14px" onclick="document.getElementById('vfy-img-input').click()">
            ...
            <span style="font-size:12px">PNG, JPG · Claude AI가 자동으로 읽습니다</span>
          </div>
          <input id="vfy-img-input" type="file" accept="image/*" style="display:none" onchange="Pages.Verify.handleImage(this)">
          <div id="vfy-img-status" style="font-size:12px;color:var(--tbl-tx-body);margin-bottom:14px"></div>

          <div style="font-size:12px;font-weight:500;text-transform:uppercase;letter-spacing:.06em;color:var(--tbl-tx-body);margin-bottom:8px;display:flex;align-items:center;gap:6px">
            <span style="width:18px;height:18px;border-radius:50%;background:var(--tx);color:var(--card);font-size:12px;font-weight:500;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0">4</span>
            검증 실행
          </div>
          <button onclick="Pages.Verify.runVerify()" style="width:100%;padding:9px;background:#185FA5;color:#fff;border:none;border-radius:var(--rs);font-size:12px;font-weight:500;cursor:pointer">검증 실행</button>
        </div>
      </div>
