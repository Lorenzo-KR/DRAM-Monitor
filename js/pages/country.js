      ...
      const bizCards = CONFIG.BIZ_LIST.map(b => {
        ...
        if (bl.length === 0 && bill === 0) return `
          <div class="co-biz-card" style="border-top-color:${CONFIG.BIZ_COLORS[b]};opacity:.45">
            <div class="co-biz-head"><span class="co-biz-name" style="color:${CONFIG.BIZ_COLORS[b]}">${CONFIG.BIZ_LABELS[b]}</span><span class="bdg b-neu" style="font-size:10px">없음</span></div>
        ...
        return `
          <div class="co-biz-card" style="border-top-color:${CONFIG.BIZ_COLORS[b]}">
            ...
            ${actB.length > 0 ? `<div class="pb-w" style="margin-top:6px"><div class="pb" style="width:${avgPct}%;background:${CONFIG.BIZ_COLORS[b]}"></div></div><div style="font-size:10px;color:var(--tx3);margin-top:2px">평균 진행률 ${avgPct}%</div>` : ''}
          </div>`;
      }).join('');
