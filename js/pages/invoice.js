...
Pages.Invoice = (() => {
  ...
  function render() {
    ...
    const ST_STYLE  = { paid: 'border:1px solid var(--bd);color:var(--tx2);background:transparent', partial: 'border:1px solid var(--bd);color:var(--tx2);background:transparent', unpaid: 'border:1px solid #FECACA;color:#dc2626;background:#FEF2F2' };
    const ST_LABEL  = { paid: '수금완료', partial: '부분수금', unpaid: '미수금' };

    function badge(txt, style) { return `<span style="display:inline-flex;align-items:center;font-size:12px;font-weight:500;padding:1px 6px;border-radius:3px;white-space:nowrap;${style}">${txt}</span>`; }
          ...
          return `
            <tr style="border-bottom:1px solid var(--bd)">
              ...
              <td style="padding:4px 8px;white-space:nowrap">
                ${r.status !== 'paid' ? `<button class="btn sm" style="font-size:12px;padding:2px 7px" onclick="Pages.Invoice.quickPaid(${r.id})">수금</button>` : ''}
                <button class="btn sm" style="font-size:12px;padding:2px 7px" onclick="Pages.Invoice.openPanel(${r.id})">수정</button>
                <button class="btn del sm" style="font-size:12px;padding:2px 7px" onclick="Pages.Invoice.delete(${r.id})">삭제</button>
              </td>
            </tr>`;
        }).join('');
