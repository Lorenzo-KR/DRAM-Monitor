...
/**
 * 레코드 ID 생성 — 세션 내 절대 중복되지 않는 13자리 정수.
 *
 * `Date.now() + Math.random()` 을 쓰면 안 된다:
 *   - 일괄 저장 루프는 같은 밀리초 안에서 다 돌기 때문에 Date.now()가 전부 같다.
 *   - Google Sheets 는 숫자를 유효자리 15자리까지만 저장한다.
 *     1787654321098.4823 → 1787654321098.48 로 잘려 소수 2자리만 남고,
 *     100개 값 중에서 뽑는 꼴이라 30여 건만 넣어도 ID 충돌이 사실상 확정된다.
 *     충돌하면 수정·삭제(`updateRow`/`deleteRow`가 첫 일치 행만 잡음)가 엉뚱한 행에 걸린다.
 * 같은 ms 안에서는 +1 씩 밀어 정수 유일성을 보장한다.
 */
